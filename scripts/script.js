...
function updateWeights(type) {
    ...
    case 'drink':
      selectedItems.forEach((item) => {
        ...
        switch (item) {
          case 'Beer':
          case 'Wine':
            individualServings = adultCount; // Adjust for special cases
            break;
          default:
            individualServings = adultCount + childCount * 0.5;
            break;
        }
        ...
        if (itemWeightSpan) {
          itemWeightSpan.textContent = `${Math.ceil(individualWeight)} Units`; // Use Math.ceil to round up
        }
      });
      break;
    default:
  ...
  weightOutput.textContent = `${Math.ceil(totalWeight)}${type === 'meat' ? 'g' : ' Units'}`;
}
